...
**Current state:** The STARK proof lands on-chain as opaque NoteData — the chain stores it but doesn't verify it. The proof is real and independently verifiable off-chain, but on-chain settlement still requires trusting the submitter. This is the last trust gap in the pipeline. It closes when Nockchain ships Intent Scripting, which will let the chain verify STARK proofs inline and make the full process trustless end-to-end.
