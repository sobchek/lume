#!/usr/bin/env bash
# ============================================================================
# Lume Live Demo — Full Pipeline on Nockchain Fakenet
#
# A single command that demonstrates the entire Lume pipeline:
...
VESSEL_PORT="${VESSEL_PORT:-3000}"
NOCKCHAIN_GRPC_ADDR="${NOCKCHAIN_GRPC_ADDR:-127.0.0.1:9090}"
# Demo signing key PKH — matches vessel::signing::demo_signing_key()
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --help|-h)
            ...
            echo "  --port PORT               Vessel HTTP port (default: 3000)"
...
cleanup() {
    local rc=$?
    echo ""
    if [[ -n "$VESSEL_PID" ]] && kill -0 "$VESSEL_PID" 2>/dev/null; then
        dim "Stopping vessel server (pid $VESSEL_PID)..."
        kill "$VESSEL_PID" 2>/dev/null || true
        # Wait up to 5s for graceful shutdown, then force kill
        local i=0
        while kill -0 "$VESSEL_PID" 2>/dev/null && [[ $i -lt 5 ]]; do
            sleep 1
            i=$((i + 1))
        done
        if kill -0 "$VESSEL_PID" 2>/dev/null; then
            dim "Force-killing vessel..."
            kill -9 "$VESSEL_PID" 2>/dev/null || true
        fi
        wait "$VESSEL_PID" 2>/dev/null || true
    fi
...
banner "Lume: Sovereign RAG on Nockchain"
...
if [[ "$USE_CHAIN" == "true" ]]; then
    ...
    while [[ $elapsed -lt $WAIT_BLOCKS_TIMEOUT ]]; do
        # Build vessel first if needed (do this while waiting)
        if [[ $elapsed -eq 0 ]]; then
            dim "Building vessel while waiting for blocks..."
            (cd "$VESSEL_DIR" && cargo build --release 2>&1 | tail -3) || true
            ok "Vessel built"
        fi

        # Try a simple gRPC connectivity check via the test binary
        if (cd "$VESSEL_DIR" && \
            LUME_FAKENET_CHAIN_ENDPOINT="$CHAIN_URL" \
    ...
    dim "Building vessel..."
    (cd "$VESSEL_DIR" && cargo build --release 2>&1 | tail -3)
    ok "Vessel built"
fi

# ---------------------------------------------------------------------------
# Step 3: Start vessel HTTP server
# ---------------------------------------------------------------------------

step "3" "Starting Lume vessel HTTP server..."

mkdir -p "$PROJECT_ROOT/.fakenet"

VESSEL_BIN="$VESSEL_DIR/target/release/vessel"
if [[ ! -x "$VESSEL_BIN" ]]; then
    # Fallback to debug build
    VESSEL_BIN="$VESSEL_DIR/target/debug/vessel"
    if [[ ! -x "$VESSEL_BIN" ]]; then
        fail "Vessel binary not found. Build with: cd vessel && cargo build --release"
        exit 1
    fi
fi

# Construct vessel flags
VESSEL_FLAGS=(
    --new
    --serve
    --port "$VESSEL_PORT"
...
if [[ -n "$OLLAMA_URL" ]]; then
    VESSEL_FLAGS+=(--ollama-url "$OLLAMA_URL" --model "$OLLAMA_MODEL")
fi
...
dim "Command: vessel ${VESSEL_FLAGS[*]}"

# Start vessel in background
"$VESSEL_BIN" "${VESSEL_FLAGS[@]}" > "$PROJECT_ROOT/.fakenet/vessel.log" 2>&1 &
VESSEL_PID=$!
dim "Vessel PID: $VESSEL_PID, log: $PROJECT_ROOT/.fakenet/vessel.log"

# Wait for the HTTP server to be ready
VESSEL_URL="http://localhost:$VESSEL_PORT"
vessel_ready=false
for i in $(seq 1 60); do
    if curl -sf "$VESSEL_URL/health" >/dev/null 2>&1; then
        vessel_ready=true
        break
    fi
    sleep 1
...
if [[ "$vessel_ready" == "true" ]]; then
    ok "Vessel HTTP server ready at $VESSEL_URL"
else
    fail "Vessel server failed to start within 60s"
    fail "Check log: $PROJECT_ROOT/.fakenet/vessel.log"
    cat "$PROJECT_ROOT/.fakenet/vessel.log" | tail -20
    exit 1
fi
...
STATUS=$(curl -sf "$VESSEL_URL/status")
...
if [[ "$HAS_TREE" == "True" ]] || [[ "$HAS_TREE" == "true" ]]; then
    ...
    INGEST_RESP=$(curl -sf -X POST "$VESSEL_URL/ingest" \
...
QUERY_RESP=$(curl -sf -X POST "$VESSEL_URL/query" \
    -H "Content-Type: application/json" \
    -d "$QUERY_BODY")
...
VESSEL_ID=$(echo "$STATUS" | python3 -c "import sys,json; print(json.load(sys.stdin)['vessel_id'])" 2>/dev/null || echo "?")

field "Vessel ID:" "$VESSEL_ID"
...
if [[ "$USE_CHAIN" == "true" ]]; then
    ...
    SETTLEMENT_COUNT=$(echo "$CHAIN_OUTPUT" | grep -o '[0-9]* Lume settlement' | head -1 | grep -o '[0-9]*' || echo "0")

    if [[ "$SETTLEMENT_COUNT" -gt 0 ]]; then
        ok "Found $SETTLEMENT_COUNT Lume settlement(s) on-chain"
        echo "$CHAIN_OUTPUT" | grep "Settlement(" | sed 's/^/    /'
    else
        dim "No Lume settlements found on-chain via test scanner."
...
echo "     - 5 NoteData entries: lume-v, lume-vid, lume-rt, lume-nid, lume-mh"
if [[ "$Q_TX_ACCEPTED" == "True" ]] || [[ "$Q_TX_ACCEPTED" == "true" ]]; then
...
echo "     POST /ingest  -> $VESSEL_URL/ingest"
echo "     POST /query   -> $VESSEL_URL/query"
echo "     GET  /status  -> $VESSEL_URL/status"
echo "     GET  /health  -> $VESSEL_URL/health"

echo ""
echo -e "${DIM}  Server still running at $VESSEL_URL (Ctrl+C or wait for cleanup)${RESET}"
echo ""
