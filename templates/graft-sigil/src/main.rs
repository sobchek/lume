...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    // --- sigil: commit note data to a Merkle tree ---

    println!("\n=== Sigil: building Merkle tree ===\n");
    let mut sigil = Sigil::new();
    let leaves: Vec<&[u8]> = notes.iter().map(|(_, v)| v.as_bytes()).collect();
    sigil.commit(&leaves);

    let root: Tip5Hash = sigil.root().expect("tree committed");
