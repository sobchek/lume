...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    // --- step 4: verify all proofs locally with Vigil ---

    println!("\n=== step 4: Vigil — local verification ===\n");
    let mut vigil = Vigil::new();
    vigil.register_root(root);

    for (i, (title, body)) in reports.iter().enumerate() {
        let proof = sigil.proof(i);
        let valid = vigil.check(body.as_bytes(), &proof, &root);
        println!("  {} '{}': {}", if valid { "ok" } else { "FAIL" }, title, valid);
    }

    // --- step 5: demonstrate settlement via Anchor ---
    //
    ...
    let tampered = vigil.check(b"Revenue down 50%. CEO arrested.", &proof, &root);
    println!("  tampered report: valid={}", tampered);

    let bad_root: Tip5Hash = [0xDEAD, 0, 0, 0, 0];
    let unreg = vigil.check(reports[0].1.as_bytes(), &proof, &bad_root);
    println!("  unregistered root: valid={}", unreg);

    println!("\n=== done ===");
    println!("\nThe Anchor pattern: submit -> commit -> register -> verify -> settle.");
