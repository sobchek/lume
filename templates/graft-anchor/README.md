...
`graft-sigil` grafts commitment and verification. This template goes further: it grafts the **Anchor** — full settlement lifecycle with replay protection. Notes transition from `%pending` to `%settled` and can never be settled twice.

Drop anchor — settle state on-chain.
...
submit reports → commit to Merkle tree → register root
                                              ↓
              verify proofs ← Vigil   settle notes ← Anchor
                                              ↓
                                    permanent record
                                    (replay-protected)
```
...
## Upgrading from graft-sigil

If you started with `graft-sigil` and need settlement:
