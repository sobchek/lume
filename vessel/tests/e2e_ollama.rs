...
//! export LUME_OLLAMA_URL="http://your-runpod:11434"
//!
//! # Optional: override model name
//! export LUME_OLLAMA_MODEL="llama3.2"
//!
//! # Run the tests
...
use vessel::llm::{self, LlmProvider, OllamaProvider};
use vessel::merkle::MerkleTree;
use vessel::noun_builder;
use vessel::retrieve::{KeywordRetriever, Retriever, SCORE_SCALE};
use vessel::types::{Chunk, Manifest, Note, NoteState, Retrieval};
...
fn ollama_url() -> Option<String> {
    std::env::var("LUME_OLLAMA_URL").ok()
}

fn ollama_model() -> String {
    std::env::var("LUME_OLLAMA_MODEL").unwrap_or_else(|_| "llama3.2".to_string())
}
...
async fn ollama_connectivity_and_response() {
    let url = match ollama_url() {
        Some(u) => u,
        None => {
            println!("SKIP: LUME_OLLAMA_URL not set");
            return;
        }
    };
...
async fn ollama_live_settlement() {
    ...
    println!("Root registered with kernel (vessel_id=7)");
...
async fn ollama_prompt_hash_consistency() {
    ...
    let hash_before = vessel::merkle::hash_leaf(prompt.as_bytes());
    ...
    let hash_after = vessel::merkle::hash_leaf(prompt.as_bytes());
