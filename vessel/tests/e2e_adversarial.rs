...
//!
//! 7 attack vectors testing the Lume settlement pipeline's security guarantees
...
async fn register_and_settle(
    ...
    let register_poke = noun_builder::build_register_poke(note.vessel, root);
...
async fn adversarial_5_replay_attack() {
    ...
    let note2 = Note { id: 2, vessel: 7, root, state: NoteState::Pending };
...
mod http_api {
    ...
    use vessel::api;
    use vessel::llm::StubProvider;
    use vessel::retrieve::KeywordRetriever;
